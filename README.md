...
경기도교육청(GOE) 메신저의 쪽지 내용을 분석하여 개인 To-Do 사이트에 자동으로 등록해주는 자동화 브릿지입니다.
...
- **가독성 기반 등록**: 개인 사이트(`todo26.site`)에 제목과 본문(요약+원문)을 줄바꿈으로 구분하여 깔끔하게 등록합니다.
...
### 3. 환경 변수 설정 (`.env`)
프로젝트 루트 폴더에 `.env` 파일을 생성하고 다음 정보를 입력합니다.
```env
GEMINI_API_KEY=your_gemini_api_key_here
TODO26_TOKEN=your_todo26_api_token_here
```
...
- `task_manager.py`: 외부 API(Todo26) 통신 모듈
