# GOE Messenger to Google Tasks Auto-Bridge

경기도교육청(GOE) 메신저의 쪽지 내용을 분석하여 Google Tasks에 자동으로 등록해주는 자동화 브릿지입니다.
...
- **가독성 기반 등록**: Google Tasks에 제목과 본문(요약+원문)을 깔끔하게 등록합니다.
...
```
2. Google Cloud Console에서 발급받은 `credentials.json` 파일을 프로젝트 폴더에 위치시켜야 합니다.
...
- `task_manager.py`: Google Tasks API 통신 모듈
- `at_decryptor.exe`: 메신저 DB 복호화용 32비트 헬퍼
- `processed_ids.json`: 처리 완료된 메시지 기록 (자동 생성)

## 주의 사항
- 깃(Git) 저장소 업로드 시 `.env`, `credentials.json`, `token.json` 및 `.db` 파일이 포함되지 않도록 주의하세요 (이미 `.gitignore`에 정의되어 있습니다).
